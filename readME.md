# The Sound of Success: An Analysis of Billboard Top 100 Trends (1958-2025)

## Haiden Hunter, Ryan Armendariz-Lopez
### hnh21@pitt.edu, raa193@pitt.edu

### Our Research Question: 
What musical characteristics are most strongly associated with chart success on the Billboard Top 100, and how have these features evolved over time? Drawing on publicly available datasets from Billboard, Spotify, and Genius, we plan to merge chart rankings with quantitative audio features such as tempo, energy, key, and danceability.

### Why This Research Question Interesting to Us:
This research topic interests us because we’re both independent music producers trying to make our way in an industry that can feel impossible to break into without a major label or massive marketing budget. We’ve always wondered what really separates songs that make it onto the Billboard charts from those that don’t. By analyzing data on popular songs, we hope to understand what kinds of musical and lyrical features tend to connect with audiences. Our goal isn’t to copy trends, but to use data to make smarter creative choices and help other indie artists do the same. 

### Why Do You Consider Your Project Useful and Who Would Benefit From Your Model:
...
Since the billboard data is accurate, it contained no accidental outliers meaning we decided to keep all of the data. As we move forward we will see if scaling will be necessary. However, based on our current plans it will not be necessary since every feature is calibrated to its own 0 to 1 scale already.

### Feature Selection:
We plan on creating a few models with different combinations of predictors for each tracks danceability, energy, loudness, key, speechiness, acousticness, liveness, valence, tempo, duration, key, and explicitness. A possible set of predictors for a baseline could be enegry, loudness, valence, tempo, and key. These features would be used to predict weeks on the chart, which would be our responce. We are still figuring out which predictors would be best. However, we choose thoes as our baseline as they are features musicials can easily control through production and mixing, allowing the artist to make fundamentally different versions of a song, since these features are comprised of the key components of a song, and use our model to predict which will perform better on the charts.

### Overall Thoughts:
We have a lot of data, which causes our visualizations to just look like blobs. Yet, there are already clear patterns appearing. We are excited to continue mssing with the data and begin to create and test our models. Also, our notebook can take up to 5 to 10 minutes to run depending on the speed of your computer so please be patient.